...
This project is pre-configured for deployment on **Render** using the `render.yaml` blueprint.
...
4. Render will automatically detect the `render.yaml` file and set up:
    - **Backend**: A Node.js Web Service (Free Tier).
    - **Frontend**: A Static Site (Free Tier).

### 3. Environment Variables
The blueprint automatically links the frontend to the backend URL. If you deploy manually, ensure the frontend has these environment variables:
- `VITE_API_URL`: Your backend URL + `/api` (e.g., `https://your-app.onrender.com/api`)
- `VITE_WS_URL`: Your backend URL with `wss://` instead of `https://` (e.g., `wss://your-app.onrender.com`)
